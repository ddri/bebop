...
export abstract class BaseContentAdapter implements ContentAdapter {
  ...
  abstract adaptContent(content: ContentInput, options: AdaptationOptions): Promise<AdaptedContent>;
  ...
  protected truncateText(text: string, maxLength: number, suffix = '...'): string {
  ...
  protected extractFirstParagraph(markdown: string): string {
    ...
    for (const paragraph of paragraphs) {
      ...
      if (cleaned.length > 50) { // Minimum meaningful length
        return cleaned;
      }
    }
  ...
  protected extractKeyPoints(content: string, maxPoints = 5): string[] {
    ...
    if (bulletMatches) {
      points.push(...bulletMatches.map(match => 
    ...
    if (numberedMatches) {
      points.push(...numberedMatches.map(match => 
    ...
    if (points.length === 0) {
      ...
      points.push(...sentences.slice(0, maxPoints).map(s => this.stripMarkdown(s.trim())));
    }
  ...
  protected createThread(content: string, maxLength: number): string[] {
    ...
    for (const sentence of sentences) {
      ...
      const potential = currentThread ? `${currentThread} ${cleanSentence}` : cleanSentence;
