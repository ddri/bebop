...
export class HashnodeClient extends BasePlatformClient {
    ...
    try {
      ...
      if (credentials.type !== 'api-key') {
        throw new AuthenticationError(
          this.platform,
          'Hashnode requires API key authentication'
        );
      }

      const parsedCredentials = HashnodeCredentialsSchema.parse(
        credentials.data
      );
      ...
      throw new AuthenticationError(
        this.platform,
        'Failed to authenticate with Hashnode'
      );
    }
  }
    ...
    try {
      ...
      if (!publicationId) {
        return this.createErrorResult(
          'Publication ID is required for publishing to Hashnode'
        );
      }
      ...
      if (!response.data?.publishPost?.post) {
        return this.createErrorResult(
          'No post data returned from Hashnode',
          response as Record<string, unknown>
        );
      }
  ...
  async update(
    ...
    try {
      ...
      // Settings
      if (
        hashnodeConfig.enableTableOfContents !== undefined ||
      ...
      // Meta tags
      if (
        hashnodeConfig.metaTitle ||
        hashnodeConfig.metaDescription ||
  ...
  async getMetadata(): Promise<PlatformMetadata> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get Hashnode metadata: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getPublicationSeries(publicationId: string): Promise<HashnodeSeries[]> {
    ...
    try {
      ...
      return response.data.publication.series.edges.map(
        (edge: { node: any }) => edge.node
      );
    } catch (error) {
      throw new Error(
        `Failed to get publication series: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async createSeries(
    ...
    try {
      ...
      throw new Error(
        `Failed to create series: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async validateToken(): Promise<void> {
    ...
    try {
      const response = await this.makeGraphQLRequest(GET_USER_QUERY, {
        username: 'me',
      });
