...
async function createFailureTestScenarios() {
  ...
  try {
    ...
    if (testResult.ok) {
      console.log('✅ Triggered scheduler check - watch server console for retry logic');
    } else {
...
async function monitorFailureTests() {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/scheduler?action=health');
    ...
    if (response.ok) {
      ...
      if (health.recentActivity && health.recentActivity.length > 0) {
        ...
        health.recentActivity.forEach((activity: any, i: number) => {
          console.log(`   ${i + 1}. [${activity.status}] "${activity.content.title}"`);
          console.log(`      → ${activity.destination.name} (${activity.destination.type})`);
          console.log(`      → Attempts: ${activity.attempts}/3`);
          if (activity.error) {
            console.log(`      → Error: ${activity.error}`);
          }
          if (activity.publishedAt) {
            console.log(`      → Published: ${new Date(activity.publishedAt).toLocaleString()}`);
          }
          console.log('');
        });
      }
...
async function testManualRetry() {
  ...
  try {
    ...
    const healthResponse = await fetch('http://localhost:3000/api/scheduler?action=health');
    ...
    const failedSchedule = health.recentActivity?.find((activity: any) => activity.status === 'FAILED');
    
    if (!failedSchedule) {
      console.log('ℹ️  No failed schedules found to retry');
      console.log('💡 Create some failure scenarios first with: npm run test:scheduler');
      return;
    }
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('  create  - Trigger scheduler check (creates failures if broken configs exist)');
