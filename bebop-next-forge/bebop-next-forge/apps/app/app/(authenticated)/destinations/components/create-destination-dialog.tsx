...
import { DestinationType, type Destination } from '@repo/database/types';
...
export const CreateDestinationDialog = ({ children, editingDestination, onClose }: CreateDestinationDialogProps) => {
  ...
  React.useEffect(() => {
    if (editingDestination) {
      const config = (editingDestination.config && typeof editingDestination.config === 'object' && !Array.isArray(editingDestination.config)) 
  ...
  const onSubmit = async (data: CreateDestinationForm) => {
    try {
      ...
      console.error(`Failed to ${isEditing ? 'update' : 'create'} destination:`, error);
    }
  };
  ...
  return (
    <Dialog open={isEditing || open} onOpenChange={(newOpen) => {
      setOpen(newOpen);
      if (!newOpen) {
        onClose?.();
      }
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isEditing ? 'Edit Publishing Destination' : 'Add Publishing Destination'}
          </DialogTitle>
          <DialogDescription>
            {isEditing 
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <SelectContent>
                      {Object.entries(platformCategories).map(([category, platforms]) => (
                        <React.Fragment key={category}>
                          ...
                          {platforms
                            .filter(platform => 
                              category === 'Coming Soon' ? false : phase1Platforms.includes(platform)
                            )
                            .map((platform) => (
                              <SelectItem key={platform} value={platform}>
                                <div className="flex items-center gap-2">
                                  {platformIcons[platform]}
                                  <span>{destinationTypeLabels[platform]}</span>
                          ...
                          {category !== 'Coming Soon' && <Separator className="my-1" />}
                        </React.Fragment>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      {selectedType === DestinationType.MASTODON && (
                        <FormDescription>
                          The URL of your Mastodon instance (without trailing slash)
                        </FormDescription>
                      )}
                      <FormMessage />
                    </FormItem>
            ...
            {fieldConfig.additionalFields && fieldConfig.additionalFields.length > 0 && (
              <>
                <Separator />
                <div className="space-y-4">
                  <h4 className="text-sm font-medium">Platform-Specific Settings</h4>
                  {fieldConfig.additionalFields.map((field) => (
                    <FormField
                      key={field.name}
                      control={form.control}
                      name={`additionalConfig.${field.name}` as `additionalConfig.${string}`}
